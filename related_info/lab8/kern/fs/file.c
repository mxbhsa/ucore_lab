...
file_open(char *path, uint32_t open_flags) {
  cprintf("system open file path=%s\n", path);
	...
	cprintf("vfs open success path=%s!\n", path);
        fd_array_free(file);
        return ret;
    }
    ...
    fd_array_open(file);
    cprintf("open file fd=%d success!\n", file->fd);
    return file->fd;
}
...
int
file_close(int fd) {
  cprintf("system close file fd=%d\n", fd);
    int ret;
    struct file *file;
    if ((ret = fd2file(fd, &file)) != 0) {
        return ret;
    ...
    fd_array_close(file);
    cprintf("close file fd=%d success!\n", fd);
    return 0;
}
...
file_read(int fd, void *base, size_t len, size_t *copied_store) {
  cprintf("system read file fd=%d\n", fd);
    ...
    *copied_store = 0;
    ...
    cprintf("vop read file which fd=%d success!\n", fd);
    ...
    *copied_store = copied;
    ...
    cprintf("read file fd=%d success!\n", fd);
    return ret;
}
...
file_write(int fd, void *base, size_t len, size_t *copied_store) {
  cprintf("system write file fd=%d\n", fd);
    int ret;
    struct file *file;
    *copied_store = 0;
    if ((ret = fd2file(fd, &file)) != 0) {
    ...
    cprintf("vop write file fd=%d success!\n", fd);
    size_t copied = iobuf_used(iob);
    if (file->status == FD_OPENED) {
        file->pos += copied;
    }
    *copied_store = copied;
    fd_array_release(file);
    cprintf("write file fd=%d success!\n", fd);
    return ret;
}
